...
static void __bst_tree_insert(bst_node_t *bst_node,
                                    void *data, size_t data_size,
                                    int (*cmp)(const void *, const void *))
{
    if (cmp(data, bst_node->data) < 0) {
        if (bst_node->left) {
            __bst_tree_insert(bst_node->left, data, data_size, cmp);
        } else {
            bst_node_t *node  = __bst_node_create(data, data_size);
            bst_node->left = node;
        } 
    } else {
        if (bst_node->right) {
            __bst_tree_insert(bst_node->right, data, data_size, cmp);
        } else {
            bst_node_t *node  = __bst_node_create(data, data_size);
            bst_node->right = node;
        }
...
void bst_tree_insert(bst_tree_t *bst_tree, void *data)
{
    ...
    __bst_tree_insert(bst_tree->root, data, bst_tree->data_size, bst_tree->cmp);
}
 ...
 */
static bst_node_t *find_predecesor(bst_node_t *bst_node) {
    bst_node_t *tmp = bst_node->left;
                                    ...
                                    void (*free_data)(void *))
{
    ...
    if (rc < 0) {
        __bst_tree_remove(bst_node->left, data, data_size, cmp, free_data);
    } else if (rc > 0) {
        __bst_tree_remove(bst_node->right, data, data_size, cmp, free_data);
    } else {
        if (!bst_node->left && !bst_node->right) {          // leaf
            free_data(bst_node->data);
            free(bst_node);

            bst_node = NULL;

        } else if (!bst_node->right) {                      // left child
            tmp = bst_node;
        ...
        } else if (!bst_node->left) {                       // right child
            tmp = bst_node;
