...
main()
{
    Treap_tree* tree = treap_create((int (*)(void*, void*))(cmp));
    ...
    for (int i = 1; i <= no_inserts; ++i) {
        scanf("%d", &value);
        treap_insert(&tree->root, &value, sizeof(int), tree->cmp);
        // Used for checker.
        if (task == 1) {
            if (check_treap(tree->root, tree->cmp) == i) {
                printf("Corect insertion. Treap has %d nodes.\n",
                       check_treap(tree->root, tree->cmp));
            } else {
                printf("%s\n", "Wrong insertion.");
            }
        }
    ...
    if (task >= 2) {
        ...
        for (int i = 1; i <= no_finds; ++i) {
            scanf("%d", &value);
            if (task == 2) {
                if (get_key(tree->root, &value, tree->cmp)) {
                    printf("%d %s\n", value, "is in Treap.");
                } else {
    ...
    if (task >= 3) {
        ...
        for (int i = 1; i <= no_deletes; ++i) {
            scanf("%d", &value);
            if (task == 3) {
                treap_delete(&tree->root, &value, sizeof(int), tree->cmp);
                // Used for checker.
                if (1) {
