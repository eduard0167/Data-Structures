...
static void ll_remove_first_node(linked_list_t* list) {
...
static void ll_remove_last_node(linked_list_t *list) {
    ll_node_t *curr = list->head;
...
void ll_remove_nth_node(linked_list_t* list, unsigned int n)
{
    ...
    if (n == 0) {
        ll_remove_first_node(list);
    }

    if (list->size <= n) {
        ll_remove_last_node(list);
    }
